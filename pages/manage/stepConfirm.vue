<template>
    <NuxtLayout name="custom">
        <div class="step">
            <!-- header -->
            <div class="step-header">
                <div class="step-header-num">1</div>
                <div class="step-header-head">基本資訊</div>
                <div class="step-header-line"></div>
                <div class="step-header-num">2</div>
                <div class="step-header-head">詳細內容</div>
                <div class="step-header-line"></div>
                <div class="step-header-num">3</div>
                <div class="step-header-head">確認內容</div>
            </div>

            <!-- body -->
            <div class="step-day-title3">基本設定</div>
            <div class="step-day-box">
                <div class="step-day-row">
                    <div class="step-day-title">劇本封面</div>
                    <div class="step-day-img"></div>
                </div>
                <div class="step-day-row">
                    <div class="step-day-title">劇本名稱</div>
                    <div class="step-day-text">虎兔篇</div>
                </div>
                <div class="step-day-row">
                    <div class="step-day-title">劇本簡述</div>
                    <div class="step-day-text">潛在霸凌者需求：生理需求</div>
                </div>
                <div class="step-day-row">
                    <div class="step-day-title">劇本時長 (天)</div>
                    <div class="step-day-text">2天</div>
                </div>
                <div class="step-day-row">
                    <div class="step-day-title">教學重點</div>
                    <div class="step-day-text">
                        讓學生認識有些人會因為不同的生理需求，例如過動症，而難以控制自己的情緒及行為。
                        當我們了解每個人的獨特之處，我們可以試著協助他人。
                    </div>
                </div>
                <div class="step-day-row">
                    <div class="step-day-title">給老師的提醒</div>
                    <div class="step-day-text">讓學生認識有些人會因為不同的生</div>
                </div>
                <div class="step-day-row">
                    <div class="step-day-title">前導說明</div>
                    <div class="step-day-text">讓學生認識有些人會因為不同的生</div>
                </div>
            </div>

            <div class="step-day-title3">基本設定</div>

            <!-- 第一日 -->
            <div class="step-day">
                <div class="step-day-head-row">
                    <div class="step-day-head">第 1 日</div>
                    <div class="step-day-up">^</div>
                </div>

                <div class="step-day-box">
                    <div class="step-day-row">
                        <div class="step-day-title">建議時間</div>
                        <div class="step-day-text">10 分鐘</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">帶領方式說明</div>
                        <div class="step-day-text">2. 在課堂上發布第一日的影片:虎兔篇day1。</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">本日劇情</div>
                        <div class="step-day-text">2. 在課堂上發布第一日的影片:虎兔篇day1。</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">額外資訊</div>
                        <div class="step-day-text">2. 在課堂上發布第一日的影片:虎兔篇day1。</div>
                    </div>

                    <div class="step-day-row">
                        <div class="step-day-title">學生問題討論</div>
                        <div class="step-day-green">
                            <div class="step-day-green-head">
                                <div class="step-day-green-question">？</div>
                                <div class="step-day-green-head">如果你是他們的同學，當下你會怎麼做?</div>
                            </div>
                            <div class="step-day-green-content">
                                <div>
                                    <div class="step-day-green-answer">A</div>
                                    <div class="step-day-green-head">溫和堅定地制止張萌虎。</div>
                                </div>
                                <div>
                                    <div class="step-day-green-sub">秩序：</div>
                                    <div class="step-day-green-text">加分</div>
                                    <div class="step-day-green-sub">關係：</div>
                                    <div class="step-day-green-text">加分</div>
                                </div>
                            </div>
                            <div class="step-day-green-content">
                                <div>
                                    <div class="step-day-green-answer">B</div>
                                    <div class="step-day-green-head">溫和堅定地制止張萌虎。</div>
                                </div>
                                <div>
                                    <div class="step-day-green-sub">秩序：</div>
                                    <div class="step-day-green-text">加分</div>
                                    <div class="step-day-green-sub">關係：</div>
                                    <div class="step-day-green-text">加分</div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="step-day-row">
                        <div class="step-day-title">家長問題討論</div>
                        <div class="step-day-green">
                            <div class="step-day-green-head">
                                <div class="step-day-green-question">？</div>
                                <div class="step-day-green-head">如果你是他們的同學，當下你會怎麼做?</div>
                            </div>
                            <div class="step-day-green-content">
                                <div>
                                    <div class="step-day-green-answer">A</div>
                                    <div class="step-day-green-head">溫和堅定地制止張萌虎。</div>
                                </div>
                                <div>
                                    <div class="step-day-green-sub">秩序：</div>
                                    <div class="step-day-green-text">加分</div>
                                    <div class="step-day-green-sub">關係：</div>
                                    <div class="step-day-green-text">加分</div>
                                </div>
                            </div>
                            <div class="step-day-green-content">
                                <div>
                                    <div class="step-day-green-answer">B</div>
                                    <div class="step-day-green-head">溫和堅定地制止張萌虎。</div>
                                </div>
                                <div>
                                    <div class="step-day-green-sub">秩序：</div>
                                    <div class="step-day-green-text">加分</div>
                                    <div class="step-day-green-sub">關係：</div>
                                    <div class="step-day-green-text">加分</div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="step-day-row">
                        <div class="step-day-title">教材檔案</div>
                        <div class="step-day-net">
                            <div class="step-day-row">
                                <div class="step-day-title">劇情影片</div>
                                <div class="step-day-add">第1日教學影片.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">學習單</div>
                                <div class="step-day-add">第1日學習單.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">教學簡報</div>
                                <div class="step-day-add">第1日教學簡報.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">額外資訊</div>
                                <div class="step-day-add">第1日額外資訊.mp4</div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>

            <!-- 結局日 -->
            <div class="step-day">
                <div class="step-day-head-row">
                    <div class="step-day-head">第 2 日 (結局日)</div>
                    <div class="step-day-up">^</div>
                </div>

                <div class="step-day-box">
                    <div class="step-day-row">
                        <div class="step-day-title">建議時間</div>
                        <div class="step-day-text">10 分鐘</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">帶領方式說明</div>
                        <div class="step-day-text">統計各組學生及家長答案</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">結局一描述 (鴞老師)</div>
                        <div class="step-day-text">統計各組學生及家長答案</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">結局一描述 (鴿老師)</div>
                        <div class="step-day-text">統計各組學生及家長答案</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">結局一描述 (鴉老師)</div>
                        <div class="step-day-text">統計各組學生及家長答案</div>
                    </div>
                    <div class="step-day-row">
                        <div class="step-day-title">結局一描述 (鷹老師)</div>
                        <div class="step-day-text">統計各組學生及家長答案</div>
                    </div>

                    <div class="step-day-row">
                        <div class="step-day-title">結局影片</div>
                        <div class="step-day-net">
                            <div class="step-day-row">
                                <div class="step-day-title">結局一 (鴞老師) 影片</div>
                                <div class="step-day-add">結局一.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">結局二 (鴿老師) 影片</div>
                                <div class="step-day-add">結局二.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">結局三 (鴉老師) 影片</div>
                                <div class="step-day-add">結局三.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">結局四 (鷹老師) 影片</div>
                                <div class="step-day-add">結局四.mp4</div>
                            </div>
                        </div>
                    </div>

                    <div class="step-day-row">
                        <div class="step-day-title">教材檔案</div>
                        <div class="step-day-net">
                            <div class="step-day-row">
                                <div class="step-day-title">學習單</div>
                                <div class="step-day-add">學習單.mp4</div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">學習簡報</div>
                                <div class="step-day-add">學習簡報.mp4</div>
                            </div>
                        </div>
                    </div>

                    <div class="step-day-row">
                        <div class="step-day-title">計分設定</div>
                        <div class="step-day-net">
                            <div class="step-day-row">
                                <div class="step-day-title">結局一 (鴞老師)</div>
                                <div class="step-day-row2">
                                    <div class="step-day-test2">秩序：</div>
                                    <div class="step-day-test3">正面</div>
                                    <div class="step-day-test2">關係：</div>
                                    <div class="step-day-test3">正面</div>
                                </div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">結局二 (鴿老師)</div>
                                <div class="step-day-row2">
                                    <div class="step-day-test2">秩序：</div>
                                    <div class="step-day-test3">正面</div>
                                    <div class="step-day-test2">關係：</div>
                                    <div class="step-day-test3">正面</div>
                                </div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">結局三 (鴉老師)</div>
                                <div class="step-day-row2">
                                    <div class="step-day-test2">秩序：</div>
                                    <div class="step-day-test3">正面</div>
                                    <div class="step-day-test2">關係：</div>
                                    <div class="step-day-test3">正面</div>
                                </div>
                            </div>
                            <div class="step-day-line"></div>
                            <div class="step-day-row">
                                <div class="step-day-title">結局四 (鷹老師)</div>
                                <div class="step-day-row2">
                                    <div class="step-day-test2">秩序：</div>
                                    <div class="step-day-test3">正面</div>
                                    <div class="step-day-test2">關係：</div>
                                    <div class="step-day-test3">正面</div>
                                </div>
                            </div>
                        </div>
                    </div>                    


                </div>
            </div>

            <!-- next -->
            <div class="step-next">
                <div class="step-next-next">完成</div>
                <div class="step-next-cancel">上一步</div>
            </div>
        </div>
    </NuxtLayout>
</template>

<script setup>


</script>

<style lang="scss" scoped>
@import '~/assets/styles/step.scss';

.box {

}
</style>